...
*   **メインコンテンツ (`<Main>`)**:
    ...
    *   **2. ユーザー情報エリア (`<UserInfo />`)
        *   **役割:** 現在ログインしているユーザーの基本情報を表示・編集する。
        *   **UI:**
            *   カード形式でユーザーの名前（`authName`）、メールアドレス（`authEmail`）を表示。
            *   ニックネーム（`nickname`）、誕生日（`birthday`）、性別（`gender`）、自己紹介文（`introduction`）の編集フォーム。
            *   プロフィール画像（`profileImageUrl`）のアップロード/変更機能。
            *   デフォルト表示するペット（`primaryPetId`）の選択ドロップダウン。
            *   アプリ設定（`settings`）として、テーマ（`theme`）と通知設定（`notifications.dailySummary`）の切り替えトグル。
