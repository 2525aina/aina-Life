...
export const UserProfileEditForm: React.FC<UserProfileEditFormProps> = ({
  ...
  const [introduction, setIntroduction] = useState(firestoreUser?.introduction || "");
  const [primaryPetId, setPrimaryPetId] = useState(firestoreUser?.primaryPetId || "");
  const [theme, setTheme] = useState(firestoreUser?.settings?.theme || "system");
  ...
  useEffect(() => {
    ...
    setDailySummaryNotifications(firestoreUser?.settings?.notifications?.dailySummary || false);
  }, [firestoreUser]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-gray-600 p-3 rounded-md">
        <p className="text-lg font-semibold">名前: {currentUser.displayName || "未設定"}</p>
        <p className="text-sm text-gray-300">メール: {currentUser.email || "未設定"}</p>
      </div>

      {/* Nickname */}
      <div>
        <label htmlFor="nickname" className="block text-sm font-medium text-gray-300">ニックネーム</label>
        <input
          type="text"
          id="nickname"
          value={nickname}
      ...
      {/* Birthday */}
      <div>
        <label htmlFor="birthday" className="block text-sm font-medium text-gray-300">誕生日</label>
        <input
          type="date"
          id="birthday"
          value={birthday}
      ...
      {/* Gender */}
      <div>
        <label htmlFor="gender" className="block text-sm font-medium text-gray-300">性別</label>
        <select
          id="gender"
          value={gender}
      ...
      {/* Introduction */}
      <div>
        <label htmlFor="introduction" className="block text-sm font-medium text-gray-300">自己紹介</label>
        <textarea
          id="introduction"
          value={introduction}
      ...
      {/* Primary Pet ID */}
      <div>
        <label htmlFor="primaryPetId" className="block text-sm font-medium text-gray-300">デフォルト表示ペット</label>
        <select
          id="primaryPetId"
          value={primaryPetId}
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="theme" className="block text-sm font-medium text-gray-300">テーマ</label>
        <select
          id="theme"
          value={theme}
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="dailySummaryNotifications" className="block text-sm font-medium text-gray-300">
          日次サマリー通知
        </label>
        <input
          type="checkbox"
      ...
      <div>
        <label htmlFor="profileImage" className="block text-sm font-medium text-gray-300">プロフィール画像</label>
        <input
          type="file"
          id="profileImage"
          accept="image/*"
